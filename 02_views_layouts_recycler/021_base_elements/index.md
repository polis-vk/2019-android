...
View это класс, который представляет собой базовый элемент UI. View занимает прямоугольную область экрана и отвечает за отрисовку и обработку событий. Все другие визуальные компоненты наследуются от View.
...
Для управления местом, занимаемым каждым элементом, используется атрибут `layout_weight`. Этот атрибут указывает "значимость" элемента. Чем больше значение, тем более высокий приоритет будет отдам элементу.
